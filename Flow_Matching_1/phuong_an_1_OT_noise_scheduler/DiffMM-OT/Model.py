...
class GaussianDiffusionOT(GaussianDiffusion):
	"""
	[Phuong an 1 - DiffMM_FlowMatching_Optimization_Plan.md]
	Thay lich trinh nhieu VP-style (eq 3, DiffMM) bang duong di OT-linear (eq 20, Flow Matching):
		mu_t    = 1 - t/(T-1)              (he so nhan alpha_0 that; t=0 gan du lieu goc, t=T-1 gan nhieu)
		sigma_t = 1 - (1 - sigma_min) * mu_t
	Giu nguyen 100%: kien truc mang Denoise (MLP), MSI/gc_loss (eq 14), khung weighted-MSE cua ELBO
	(training_losses duoc ke thua nguyen ven tu lop cha, khong sua 1 dong nao), top-k rebuild (D4),
	toan bo D5 (Cross-Modal Contrastive Augmentation) / D6 (Multi-Modal Graph Aggregation) /
	D7 (Multi-Task Training).
	Chi override 3 ham: q_sample (forward), p_mean_variance (reverse), va SNR (de trong so cua
	training_losses duoc tinh dung tren duong OT thay vi gia dinh variance-preserving cua DDPM goc).
	Ly do bat buoc phai doi ca 3: cong thuc DDPM goc rang buoc mu^2 + sigma^2 = 1 (variance-preserving),
	trong khi duong OT (mu_t=1-t/(T-1), sigma_t=1-(1-sigma_min)*mu_t) khong con rang buoc nay, nen
	khong the tai dung nguyen si alphas_cumprod/betas cua lop cha.
	SNR duoc clip o snr_clip (mac dinh 50, giong ky thuat "Min-SNR weighting" pho bien) de trong so
	loss khong bi "no" gan buoc t=0 (noi mu_t=1 gan nhu tuyet doi, SNR that = mu^2/sigma^2 co the rat lon
	neu sigma_min nho) — neu khong clip, mot vai buoc t co the co trong so lon gap hang chuc nghin lan
	cac buoc con lai, gay mat on dinh khi huan luyen.
	"""

	def __init__(self, sigma_min, steps, snr_clip=50.0):
	...
	def SNR(self, t):
		# SNR "that" cua duong OT: ty le (tin hieu/nhieu)^2 = mu_t^2 / sigma_t^2, duoc clip de on dinh
		mu_t = self.mu_coef[t]
		sigma_t = self.sigma_coef[t].clamp(min=1e-6)
		return (mu_t ** 2 / sigma_t ** 2).clamp(max=self.snr_clip)
	...
	def p_mean_variance(self, model, x, t):
		...
		mu_prev_raw = self._extract_into_tensor(self.mu_coef, t_prev, x.shape)
		sigma_prev_raw = self._extract_into_tensor(self.sigma_coef, t_prev, x.shape)

		is_last_step = (t == 0).float().view(-1, *([1] * (len(x.shape) - 1)))
		# t==0 la buoc khu nhieu cuoi cung -> coi "buoc truoc" la du lieu sach hoan toan (mu=1, sigma=0)
		mu_prev = is_last_step * 1.0 + (1 - is_last_step) * mu_prev_raw
		sigma_prev = is_last_step * 0.0 + (1 - is_last_step) * sigma_prev_raw

		model_mean = mu_prev * model_output + sigma_prev * noise_pred
		model_log_variance = None  # bien the OT chi ho tro suy luan tat dinh (sampling_noise=False)
		return model_mean, model_log_variance
