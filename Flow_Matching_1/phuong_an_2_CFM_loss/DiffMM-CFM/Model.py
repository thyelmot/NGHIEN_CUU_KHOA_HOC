...
class GaussianDiffusionCFM(GaussianDiffusion):
	"""
	[Phuong an 2 - Phuong_An_2_CFM_Loss_KeHoachChiTiet.md]
	Giu nguyen 100% duong di diffusion VP-style (D1: get_betas/calculate_for_diffusion/q_sample ke
	thua nguyen ven tu lop cha) va tham so hoa du doan alpha_0 (data-prediction: p_mean_variance/
	p_sample ke thua nguyen ven). Chi doi CACH TINH TRONG SO cua loss trong training_losses: tu
	w_ELBO(t) = SNR(t-1) - SNR(t) (suy tu KL divergence, gia dinh variance-preserving) sang
	w_CFM(t) = [mu_t' - (sigma_t'/sigma_t)*mu_t]^2 (suy truc tiep tu CFM loss cua Flow Matching,
	Theorem 3 - xem CT-1..CT-4 trong Phuong_An_2_CFM_Loss_KeHoachChiTiet.md, da kiem chung bang
	dai so + so hoc rang: neu mang van du doan alpha_0 (khong doi tham so hoa), CFM loss thu gon
	dung bang w_CFM(t) * ||x_hat0 - x0||^2, tuc la CHINH LA MSE hien co, chi khac 1 he so trong so).

	w_CFM duoc tinh 1 lan luc khoi tao (giong posterior_variance... cua lop cha) va CLIP o w_clip
	(kieu "Min-SNR weighting") de tranh "no" gan buoc t=0, noi sigma_t rat nho (cung nguyen nhan da
	gap va da va o Phuong an 1 khi tinh SNR that cua duong OT).
	"""

	def __init__(self, noise_scale, noise_min, noise_max, steps, beta_fixed=True, w_clip=50.0):
		super(GaussianDiffusionCFM, self).__init__(noise_scale, noise_min, noise_max, steps, beta_fixed)
		self.w_clip = w_clip
		if noise_scale != 0:
			self._precompute_cfm_weight()

	def _precompute_cfm_weight(self):
		mu = self.sqrt_alphas_cumprod              # mu_t, da co san tu lop cha (D1 khong doi)
		sigma = self.sqrt_one_minus_alphas_cumprod  # sigma_t, da co san tu lop cha (D1 khong doi)

		# sai phan lui: chi so t cua DiffMM tang theo huong NHIEU tang (t=0 gan du lieu, t=T-1 gan
		# nhieu), nguoc huong voi quy uoc t cua Flow Matching (t=0 nhieu, t=1 du lieu) - "tien ve
		# phia du lieu" trong DiffMM tuong ung voi chi so t GIAM, nen dao ham roi rac lay theo
		# huong (gia_tri_tai_t-1) - (gia_tri_tai_t).
		...
		w[0] = 1.0  # bien t=0: dung dung quy uoc da co san cua DiffMM (xem training_losses goc)
	...
	def training_losses(self, model, x_start, itmEmbeds, batch_index, model_feats):
		...
		model_output = model(x_t, ts)  # KHONG doi - mang van du doan alpha_0
