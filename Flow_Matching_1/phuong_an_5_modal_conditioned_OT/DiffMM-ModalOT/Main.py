...
from Model import Model, GaussianDiffusion, GaussianDiffusionModalOT, Denoise
...
class Coach:
	...
	def prepareModel(self):
		...
		self.diffusion_model = GaussianDiffusionModalOT(args.sigma_min, args.steps, kappa=args.kappa, g_min=args.g_min, g_max=args.g_max, w_clip=args.w_clip, use_msi=bool(args.use_msi)).cuda()  # [Phuong an 5 - Giai doan B] duong OT dieu kien-modal; kappa=0 (mac dinh) => trung khit Phuong an 1
	...
	def trainEpoch(self):
		...
		with torch.no_grad():
			...
			for _, batch in enumerate(diffusionLoader):
				...
				# image
				denoised_batch = self.diffusion_model.p_sample(self.denoise_model_image, batch_item, image_feats, args.sampling_steps, args.sampling_noise)  # [Phuong an 5] them modal_embeds (tai su dung image_feats da co san trong scope)
				...
				# text
				denoised_batch = self.diffusion_model.p_sample(self.denoise_model_text, batch_item, text_feats, args.sampling_steps, args.sampling_noise)  # [Phuong an 5] them modal_embeds
				top_item, indices_ = torch.topk(denoised_batch, k=args.rebuild_k)
				...
				if args.data == 'tiktok':
					# audio
					denoised_batch = self.diffusion_model.p_sample(self.denoise_model_audio, batch_item, audio_feats, args.sampling_steps, args.sampling_noise)  # [Phuong an 5] them modal_embeds
					top_item, indices_ = torch.topk(denoised_batch, k=args.rebuild_k)
