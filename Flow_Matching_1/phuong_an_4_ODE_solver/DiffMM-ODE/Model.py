...
class GaussianDiffusionFastSample(GaussianDiffusion):
	"""
	[Phuong an 4 - Phuong_An_4_ODE_Solver_KeHoachChiTiet.md]
	Chi doi D4 (suy luan) — D1 (forward, VP-style goc), D2 (ELBO loss goc), D3 (MSI) GIU NGUYEN
	TUYET DOI, ke thua nguyen ven tu GaussianDiffusion qua super().__init__() (khong tu dung lai
	bat ky mang he so nao — khac Phuong an 1/3 phai tu tinh lai mu/sigma do doi duong di).

	Thay vong lap T buoc tuan tu (dung cong thuc hau nghiem Bayes posterior_mean_coef1/2, CHI
	dung toan hoc cho buoc lien ke t->t-1) bang lich trinh rut gon K<T buoc, dung cong thuc DDIM
	tong quat hoa (dung lai y nguyen tu Phuong an 1/3, ap len mu/sigma CO SAN cua duong VP goc:
	sqrt_alphas_cumprod, sqrt_one_minus_alphas_cumprod) — day chinh la loi giai dong, chinh xac
	cua "probability flow ODE" cua duong VP (Song et al. 2021 - Score-Based Generative Modeling
	through SDEs), khong phai xap xi Euler/Midpoint so hoc.

	Da kiem chung: dung cong thuc posterior_mean_coef1/2 GOC nhung chi doi tap chi so t duoc lap
	qua ("skip tho") la SAI ve mat ly thuyet (cong thuc do chi hop le cho dung 1 buoc lui t->t-1),
	du test voi denoiser hoan hao khong lo ra loi nay (do dang thuc coef1+coef2=1 khien x0 luon la
	diem bat dong bat ke cong thuc nao). Vi vay KHONG dung "skip tho", ma reformulate hoan toan
	sang dang du doan-nhieu (noise-prediction) truoc khi rut gon, giong het cach da lam o q_sample
	cua duong OT (Phuong an 1/3) nhung ap cho mu/sigma cua duong VP.
