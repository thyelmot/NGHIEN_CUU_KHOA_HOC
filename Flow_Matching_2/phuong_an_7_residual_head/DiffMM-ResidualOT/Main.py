...
from Model import Model, GaussianDiffusionResidualOT, Denoise
...
class Coach:
	...
	def run(self):
		...
		for ep in range(0, args.epoch):
			...
			if tstFlag:
				...
				# [Folder_Base - Buoc 2b] Early stopping tuy chon (khong anh huong hanh vi khi patience=0)
	...
	def prepareModel(self):
		...
		self.diffusion_model = GaussianDiffusionResidualOT(args.sigma_min, args.steps, w_clip=args.w_clip, num_sample_steps=args.num_sample_steps, anchor_w=args.anchor_w, residual_head=bool(args.residual_head)).cuda()  # [Phuong an 7 (v2)] OT+CFM (PA3) + diem neo (PA6) + residual head; residual_head=0 mac dinh => trung khit Phuong an 6
	...
	def trainEpoch(self):
		...
		for i, batch in enumerate(diffusionLoader):
			...
			uEmbeds_batch = uEmbeds[batch_index]  # [Phuong an 6/7] dung de tinh diem neo alpha_l (CT-6.1)
		...
		with torch.no_grad():
			...
			for _, batch in enumerate(diffusionLoader):
				...
				# image
				denoised_batch = self.diffusion_model.p_sample(self.denoise_model_image, batch_item, uEmbeds[batch_index], iEmbeds, args.sampling_steps, args.sampling_noise)  # [Phuong an 6/7] them diem neo
				...
				# text
				denoised_batch = self.diffusion_model.p_sample(self.denoise_model_text, batch_item, uEmbeds[batch_index], iEmbeds, args.sampling_steps, args.sampling_noise)  # [Phuong an 6/7] them diem neo
				top_item, indices_ = torch.topk(denoised_batch, k=args.rebuild_k)
				...
				if args.data == 'tiktok':
					# audio
					denoised_batch = self.diffusion_model.p_sample(self.denoise_model_audio, batch_item, uEmbeds[batch_index], iEmbeds, args.sampling_steps, args.sampling_noise)  # [Phuong an 6/7] them diem neo
					top_item, indices_ = torch.topk(denoised_batch, k=args.rebuild_k)
